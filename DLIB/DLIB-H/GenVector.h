 ...
 * @warning if both _initialCapacity and _blockSize are zero function will return NULL or failed to allocate memory.
 */
 ...
 * @retval VECTOR_OVERFLOW - vector is full and blocksize is 0
 * @retval VECTOR_UNITIALIZED_ERROR - one of the pointers is NULL
 * @retval VECTOR_ALLOCATION_ERROR - failed to realloc when vector is full
 */
 ...
 * @return success or error code 
 * @retval VECTOR_SUCCESS on success 
 * @retval VECTOR_UNDERFLOW - Vector is empty
 * @retval VECTOR_UNITIALIZED_ERROR - one of the pointers is NULL
 * @warning _item can't be null. this will be assertion violation
 * @warning doesn't give warning if shrink realloc failed
 */
 ...
 * @retval VECTOR_SUCCESS on success 
 * @retval VECTOR_UNITIALIZED_ERROR - one of the pointers is NULL
 * @retval VECTOR_INDEX_OUT_OF_BOUNDS_ERROR - index is out of vector's range
 */
 ...
 * @return success or error code 
 * @retval VECTOR_SUCCESS on success 
 * @retval VECTOR_UNITIALIZED_ERROR - one of the pointers are NULL
 * @retval VECTOR_INDEX_OUT_OF_BOUNDS_ERROR - index is out of vector's range
 */
 ...
 * @param[in] _vector - Vector to use.
 * @return  capacity of vector;  0 if vector is invalid
 */
 ...
 * @returns The index in which the iteration stopped (returns 0 if _vector or _action are NULL)
 * equevallent to:
