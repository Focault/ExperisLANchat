...
/* returns TRUE to switch and FALSE to keep in that order */
typedef int	(*Qriteria)(const void *_candidate, const void *_context);

/**  
 * @brief Dynamically create a new heap  
 * @param[in] _vector - Vector that hold the elements, must be initialized
 * @param[in] _pfLess - A Qriteria comparator to be used to compare elements 
 ...
 * @retval HEAP_NOT_INITIALIZED  error, heap or element not initilized 
 * @retval HEAP_REALLOCATION_FAILED error, heap could not reallocate underlying vector 
 * @retval HEAP_OVERFLOW error, underlying vector is full and fixed size
 */
 ...
 * @return pointer to element, can be null if heap is empty (or pointer is NULL). 
 */
 ...
 * @param[in] _heap - Heap to extract from.
 * @return number of elements or zero if empty (or pointer is NULL). 
 */
 ...
 * @param[in] _act - User provided function pointer to be onvoked for each element - returns 1 to continue and 0 to stop
 * @param[in] _context - (optional) - context for _act function
 * @returns "index" at which iteration stopped. 
 * {i.e. if stopped after acting on first element returns 0,
 *  if after the last returns (size - 1),
 *  if didn't recieved stop signal returns size}
 * @warning if _heap or _act are NULL returns 0
 */
