...
#define GROUP_MAGIC_NUMBER 59312
#define MAX_NUM_MEMBERS 1000
...
{
  Group* group;
  if(_groupName == NULL || _udpIP == NULL || _port == 0
  )
   {
     return NULL;
   }
...
{
   if((Group*)_group != NULL && ((Group*)_group) -> m_magicNum 
   == GROUP_MAGIC_NUMBER)
     {
...
GroupResult GroupIncreaseSize(Group *_group)
{
   ...
   if(_group == NULL)
    ...
    if(_group -> m_size == MAX_NUM_MEMBERS)
     {
